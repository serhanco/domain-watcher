    ...
    def check_domain(self):
        try:
            ...
            if w.domain_name is None:
                print(f"[{datetime.datetime.now()}] {self.domain} ALINABİLİR ✅")
                self.domain_available = True
            else:
                print(f"[{datetime.datetime.now()}] {self.domain} HENÜZ DOLMAMIŞ ❌ (Expire: {w.expiration_date})")
                self.domain_available = False
        except Exception as e:
            print(f"[{datetime.datetime.now()}] {self.domain} ALINABİLİR olabilir ✅ (Hata: {e})")
            self.domain_available = True
    ...
    def maybe_notify(self):
        ...
        if (
            ...
            body = f"{self.domain} şu anda ALINABİLİR görünüyor!\n\n" \
                   f"Bu {self.notifications_sent + 1}. bildirimdir."
